...
description: "当需要分析某个结果远超预期或远低于预期的现象（暴涨、暴跌、集体疯狂）；评估某个决策或情境中\"多种不利因素同时爆发\"的系统性风险时调用。核心能力：Lollapalooza 效应识别。关键触发：多种不利因素同时爆发、聪明人集体做了蠢事、为什么结果比预想的严重/极端得多?、不可能只是因为这一个原因...。不适用于：现象可以用单一清晰原因充分解释时（不要过度复杂化）、只涉及温和的、渐进的变化，没。"
