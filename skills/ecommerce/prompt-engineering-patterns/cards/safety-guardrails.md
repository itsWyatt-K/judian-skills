...
description: "当需要设计 AI 产品的内容策略和拒绝规则；产品涉及敏感领域（教育、医疗、金融、法律）时调用。核心能力：安全防线与伦理边界设计。关键触发：之间的张力 ### 语言信号 - 、工具能不能用、话题能不能讨论、我不会做 X。不适用于：AI 产品不涉及任何敏感内容（如内部代码辅助工具，且不处理用户个人信息）、试图用。"
