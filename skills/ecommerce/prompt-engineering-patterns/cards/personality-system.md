...
description: "当需要产品需要提供\"语气切换\"或\"性格选择\"功能；不同用户群对同一产品有截然不同的风格偏好时调用。核心能力：可插拔人格系统设计。关键触发：语气切换、性格选择、把自己的风格带到用户文本里、用户可以选择 AI 的性格。不适用于：AI 产品只需要一种固定风格（直接将风格写入 persona-design 即可。"
