---
name: persona-design
description: "当需要从零开始设计一个新 AI 产品的 system prompt；将通用模型适配到垂直场景（医疗、法律、教育、编程等）时调用。核心能力：身份与人格定义模式。关键触发：AI 不知道自己是谁、角色混乱、帮我写一个 AI 助手的角色设定、我们的 AI 产品需要一个身份定义。不适用于：只需要调整语气风格而不改变角色定位（使用 personality-system）。"
