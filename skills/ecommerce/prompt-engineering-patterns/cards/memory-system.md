---
name: memory-system
description: "当需要AI 需要跨会话记住用户偏好、历史或上下文；产品需要\"个性化\"体验但不确定如何实现记忆架构时调用。核心能力：记忆与个性化架构。关键触发：AI 需要记住用户的偏好、跨会话保持上下文、用户说'记住这个'时该怎么处理、记忆中包含敏感信息。不适用于：AI 产品不需要任何跨会话持久化（纯一次性对话）、记忆需求实际上可以通过简单的。"
