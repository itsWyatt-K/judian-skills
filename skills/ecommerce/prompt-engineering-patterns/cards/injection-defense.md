...
description: "当用户说\"防止用户通过文档注入指令\"、\"需要信任边界设计\"、\"外部内容不应该能控制系统行为\"时调用。核心能力：注入防御与安全架构。关键触发：防止用户通过文档注入指令、需要信任边界设计、外部内容不应该能控制系统行为、如何防止越狱攻击。不适用于：纯内部 API 调用场景，所有输入均来自受控系统、已通过沙箱隔离实现的执行环境安。"
tags: ["安全", "注入防御", "提示工程", "信任边界", "系统提示"]
metadata:
  source_book: "系统提示词设计模式库"
  attribution: "Methodological framework re-derived and rewritten by the Judian project from published-book methodology notes; inspired by the cited books, no original expression reproduced."
  source_card: "classics\\系统提示词设计模式库\\整理skill\\injection-defense\\SKILL.md"
evidence: E4
