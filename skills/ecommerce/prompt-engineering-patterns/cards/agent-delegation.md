...
description: "当用户说\"不同类型的任务需要不同的专家来处理\"、\"子代理的输出质量不可控\"、\"用户不应该看到中间的思考过程\"时调用。核心能力：多代理与委派模式。关键触发：不同类型的任务需要不同的专家来处理、子代理的输出质量不可控、用户不应该看到中间的思考过程、需要一个规划→评审→执行的完整流程。不适用于：单代理系统——只有一个执行者，无需委派架构、任务类型高度单一——不需要专业化子代。"
