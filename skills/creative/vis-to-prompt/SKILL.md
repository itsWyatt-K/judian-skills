...
metadata:
  version: 1.0.0
  source: 《以眼说话：影像视觉原理及应用》 Bruce A. Block 著 / 汪代岚 译
  license: 版权书籍（仅蒸馏方法论；映射表为自撰，未复制原文表达）
...
《以眼说话》把画面拆成七元素（空间/线条/形状/影调/色彩/运动/节奏），加一根总阀（对比 vs 相似）。这套体系解决的是"画面该怎么设计"，但 FLUX.2 和 H3 不懂"有限空间""影调同步"这些电影术语——它们吃的是具象描述词。本卡做翻译层：把 vis 系列卡产出的视觉决策，逐项转成模型能理解的短语，直接嵌入 H3 六段时间轴各段或 FLUX 四层结构。

翻译原则：**电影概念 → 视觉效果描述词**，不照搬术语名。例如"有限空间"不写成 "limited space"（模型会理解为空白区域），而写成 "framed within a doorway, shallow depth of field, subject enclosed by foreground elements"（模型能还原框中框的纵深感）。
